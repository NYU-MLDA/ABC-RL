...
class LogicSynthesisEnv(gym.Env, StaticEnv):
    ...
    def extract_init_graph(self,state):
        ...
        for nodeIdx in range(numNodes):
            ...
            if nodeType == 0 or nodeType == 2:  # 0 is const 1 and 2 is PI
                data['node_type'][nodeIdx] = 0            
            elif nodeType == 1:                 # 1 is PO
            ...
            if (aigNode.hasFanin0()):
                ...
                edge_attr_val.append(0)
            if (aigNode.hasFanin1()):
                fanin = aigNode.fanin1()
                edge_src_index.append(nodeIdx)
                edge_target_index.append(fanin)
                edge_attr_val.append(1)
        ...
        data['edge_attr'] = torch.tensor(edge_attr_val,dtype=torch.long)
        #data['edge_index'] = data['edge_index'].t().contiguous()
        #print("After")
        #print(data['edge_index'].size())
        data['forward_level'], data['forward_index'], data['backward_level'], data['backward_index'] = return_order_info(data['edge_index'],numNodes)
